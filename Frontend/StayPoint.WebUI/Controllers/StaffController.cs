...
namespace StayPoint.WebUI.Controllers
{
    public class StaffController : Controller
    {
        ...
        public async Task<IActionResult> CreateStaff(CreateStaffDto createStaffDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7260/api/Staffs", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7260/api/Staffs/{id}");
        ...
        public async Task<IActionResult> UpdateStaff(int id)
        {
            ...
            var responseMessage = await client.GetAsync($"https://localhost:7260/api/Staffs/{id}");
        ...
        public async Task<IActionResult> UpdateStaff(UpdateStaffDto updateStaffDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7260/api/Staffs", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
